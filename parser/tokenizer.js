Karol.Tokenizer = class {
  ...
  stringToken (quote) {
    ...
    while (ch = iter.next()) {
      if (!ch) {
        throw new Karol.SyntaxError(`Unterminated string literal`, iter.getCurrentPosition())
      }
      if (escape) {
        escape = false
        if (ch === 'b') {
    ...
    return this.createToken(Karol.Token.TOKEN_TYPE_STRING, str, iter.getCurrentPosition())
  }
  ...
  identifierToken (start) {
    ...
    return this.createToken(Karol.Token.TOKEN_TYPE_IDENTIFIER, name, iter.getCurrentPosition())
  }
  ...
  numberToken (start) {
    ...
    return this.createToken(Karol.Token.TOKEN_TYPE_NUMBER, Number(number), iter.getCurrentPosition())
  }
  ...
  keyWordToken (start) {
    ...
    return this.createToken(Karol.Token.TOKEN_TYPE_KEY_WORD, name, iter.getCurrentPosition())
  }

  createToken (type, value, position) {
    position.column -= value.length // TODO: doesn't work properly
    return new Karol.Token({
      type,
      value,
      position
    })
  ...
  tokenize (string) {
    const iter = this.iter = new Karol.StringIterator(string)
    const tokens = []
    let ch = ''
